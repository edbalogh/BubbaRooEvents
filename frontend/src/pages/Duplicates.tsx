...
import { api, type FlaggedDuplicateItem } from '../api/client'

export default function Duplicates() {
  const [duplicates, setDuplicates] = useState<FlaggedDuplicateItem[]>([])
            ...
            <div key={dup.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <div className="flex items-start gap-6">
                <div className="flex-1">
                  <div className="text-xs text-gray-400 font-medium uppercase mb-1">Event A (keep)</div>
                  <div className="font-semibold text-gray-900">{dup.title}</div>
                  <div className="text-xs text-gray-400 mt-1 font-mono">{dup.id}</div>
                </div>
                <div className="flex-1">
                  <div className="text-xs text-gray-400 font-medium uppercase mb-1">Event B (merge into A)</div>
                  <div className="text-xs text-gray-400 mt-1 font-mono">{dup.flagged_duplicate}</div>
                </div>
              </div>
              <div className="flex gap-3 mt-4 pt-4 border-t border-gray-100">
                ...
                >
                  {merging === dup.id ? 'Merging...' : 'Merge (keep A)'}
                </button>
                <button
